...
svc = SVC(kernel='rbf')
svc.fit(X_train_compressed, y_train_encoded)
...
for i, class_label in enumerate(np.unique(y_train_encoded)):
    ax.scatter(X_train_compressed_2d[y_train_encoded == class_label, 0],
               ...
               marker='x', edgecolors='black', label=f'Train {class_label}')
...
for i, class_label in enumerate(np.unique(y_test_encoded)):
    ax.scatter(X_test_compressed_2d[y_test_encoded == class_label, 0],
               ...
               marker='s', edgecolors='black', label=f'Test {class_label}')

# Post-processing
ax.set_xlabel('First Principal Component')
ax.set_ylabel('Second Principal Component')
ax.set_title('SVM Classification with RBF Kernel (First 2 PCA Components)')
ax.legend(loc='best')
